...
| `LOG_PRETTY` | (unset → pretty) | When **unset**, logs are human-readable and colorful (ideal for local dev). Set to `false` or `0` for one-JSON-line-per-log (e.g. production, aggregators). |
